...
import type { AnalyticsReport, AnalyticsScheduleRecommendationReport, Target } from '@/lib/types';
...
const toPercent = (value: number | null | undefined) => `${((value || 0) * 100).toFixed(1)}%`;
...
const AnalyticsPage = () => {
  ...
  const { data: scheduleRecommendations } = useQuery<AnalyticsScheduleRecommendationReport>({
  ...
  const captureAudience = useMutation({
    ...
    onError: (error: unknown) => {
      ...
      alert(`Failed to capture audience snapshots: ${message}`);
    }
  });
            ...
            <Card>
              ...
              <CardContent>
                ...
                <div className="mt-2 text-xs text-muted-foreground">
                  {report.dataQuality.inboundRowsScanned} inbound rows scanned
                </div>
              </CardContent>
            </Card>
          </div>
