...
const scheduleRoutes = () => {
  ...
  router.post('/:id/dispatch', async (req, res) => {
    try {
      ...
      if (!schedule.feed_id) {
        res.status(400).json({ error: 'Schedule has no feed selected' });
        return;
      }

      if (!schedule.template_id) {
        res.status(400).json({ error: 'Schedule has no template selected' });
        return;
      }

      if (!Array.isArray(schedule.target_ids) || schedule.target_ids.length === 0) {
        res.status(400).json({ error: 'Schedule has no targets selected' });
        return;
      }
      ...
      if (!pendingCount) {
        res.status(409).json({
          error: 'No pending items to dispatch. Refresh the feed or verify the schedule feed/targets.'
        });
        return;
      }
