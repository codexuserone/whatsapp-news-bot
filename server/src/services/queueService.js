...
const queueExistingItemsForSchedule = async (scheduleId, { limit = 200 } = {}) => {
...
const sendQueuedForSchedule = async (scheduleId, whatsappClient) => {
  ...
  try {
    ...
    const { queued } = await queueExistingItemsForSchedule(scheduleId, { limit: 200 });
    if (queued > 0) {
      logger.info({ scheduleId, queued }, 'Backfilled queued messages before dispatch');
    }
